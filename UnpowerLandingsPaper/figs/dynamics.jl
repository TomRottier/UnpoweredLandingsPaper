...
function dynamics_phasespace()
        ...
        for (j, pa) in enumerate(pas)
            ...
            traj_axs = [
                (k, 1) => trajectory_axis(
                    [
                        trajs[k];
                    ]; ylabel=k == 2 ? L"altitude $z / \hat{l}$" : "", xlabel=k==3 ? L"range $x / \hat{l}$" : "", xticksvisible=false, xticklabelsvisible=false, yticksvisible=false, yticklabelsvisible=false, xlabelsize=10pt, ylabelsize=10pt, ylabelpadding=3, xlabelpadding=0, yaxisposition=:left
                ...
                (i, j) => S.GridLayout(
                    [
                        (1, 1) => phasespace_axis(
                            ...
                            ]; extend, xlabel, ylabel, xticks, yticks, xticklabelspace=5.0, yticklabelspace=15.0, xlabelpadding=-5
                        ),
                        (1, 1) => S.GridLayout(
                        ...
                        (1, 1) => S.Label(
                            text=axlabels1[i] * axlabels2[j], font=:bold, fontsize=14pt, tellheight=false, halign=0.0, tellwidth=false, padding=0, valign=1.2
                        ),
                        (1, 1) => S.Label(text=L"\alpha = %$(round(Int, rad2deg(α)))^{\circ}", fontsize=12pt, tellheight=false, tellwidth=false, valign=1.13, halign=1.0),
                    ],
    ...
    rowgap!(f.layout.content[1].content, 10pt)
    colgap!(f.layout.content[1].content, 10)

    for i in eachindex(gls)
        rowgap!(f.layout.content[1].content.content[i].content.content[4].content, 5)
        colgap!(f.layout.content[1].content.content[i].content, 2)

        # add zoom inset for certain trajectories in physical space
        if i in [1, 2, 7, 8]
            idx = 8(i - 1) .+ [4, 5, 6] # indices of trajectory axis
