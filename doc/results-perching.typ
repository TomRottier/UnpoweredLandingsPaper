...
$
which is the _time_ spent stalled, with no representation of speed. The solutions which minimise stall time lead to much larger touchdown speeds because larger speeds reach the landing site quicker, reducing the time spend stalled (red and purple lines @fig-perching\A). As some combination of stall and touchdown speed likely influence flight behaviour, we introduce a new cost function, which accounts for these effects independently from one another:
$
