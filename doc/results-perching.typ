...
The drivers of perching behaviour across all scales of birds may be further influenced by the ability for low-speed flapping flight. For larger birds, low-speed flight is not commonly observed and previous explanations have suggested this is due to the adverse scaling of available power @pennycuickPowerRequirementsHorizontal1968 or lift production @ellingtonLimitationsAnimalFlight1991. Larger birds may therefore have no option but to adopt the fixed-wing perching behaviour examined here. Smaller birds which have a great capacity for low-speed flight, may instead opt for a fixed-wing strategy over a flapping-wing one as it would require less energy and may explain why such a strategy is learned behaviour @kleinheerenbrinkOptimizationAvianPerching2022.


#figure(
  image("../figs/perching.pdf"),
  // box(width: 100%, height: 150pt, fill: luma(240), stroke: 1pt + black),
  caption: [(A) Optimal solutions for minimum touchdown speed ($lambda = 1$, green), post-stall time ($lambda=0$, red), and post-stall distance (purple) with a $C_(T"max") = 0.8$. Dashed horizontal line on $alpha$ plot is the stall angle (20 degrees). (B) Variation in touchdown speed with $lambda$, for increasing values of #ctmax from 0.8 to 1.4. (C)-(F) flight trajectories for the optimal solutions with different values of $lambda$ and #ctmax. The touchdown speeds of these solutions correspond to the curves in (B). Dotted black markers show the experimental data @kleinheerenbrinkOptimizationAvianPerching2022
  ],
)
<fig-perching>
