...
\end{equation}
which is the \emph{time} spent stalled, with no representation of speed. The solutions which minimise stall time lead to much larger touchdown speeds because larger speeds reach the landing site quicker, reducing the time spent stalled (red and purple lines in Fig.~\ref{fig-perching}A).
...
\end{equation}
where $\lambda$ is the weighting factor between the two. By varying the weighting factor the influence of stall effects and speed effects on the trajectory can be considered (Fig.~\ref{fig-perching}B).
