...
Comparison with the point mass model can be made by specifying the control input, $lambda$, such that the rigid body model has the same angle of attack time history as the point mass model. If the required moment arm could be realistically attained by the rigid body model then it can be assumed that the point mass is a valid approximation.
...
This equation gives the required moment arm to produce an angular acceleration of the body that results in the desired pitch angle and angle of attack.

@fig-rigidbody-comparison shows the two models behaving identically when the angle of attack is matched. For the lowest touchdown speed (0.1; blue/yellow trajectory in @fig-rigidbody-comparison), the moment arm value exceeds 500 towards the end of the trajectory, due to the low speeds and therefore low aerodynamic forces requiring extremely large moment arms to produce the necessary pitch acceleration. Such values would be unrealistic for any sized bird and therefore the angle of attack strategy could not be exactly followed. However, deviations in the angle of attack have only a minor effect on the resulting flight trajectory and touchdown speeds as the aerodynamic forces are so low at this point that they have a minimal effect on the dynamics. In @fig-rigidbody-comparison $lambda$ has been clamped between $plus.minus 10$ and the resulting effects on touchdown speed are not noticeable.

Conservative estimates of a pigeon sized bird ($m = 0.4 thin "kg"$, $S = 0.04 thin "m"^2$, $I = 0.02 thin "kg"thin"m"^2$), with a $lambda = 10$ corresponds to CoP shifts on the order of $1 thin "cm"$, which is easily attainable through sweeping the wings, making a point mass model suitable to capture the flight dynamics.

The required moment arm shifts are so small that it is unlikely that any effect of sweeping the wings on the aerodynamics are likely minimal, such shifts can easily be attained through sweeping (rotating the wings) rather than linearly translating, and any shift in the CoP on the wing due to changing angle of attack can easily be accounted for. Overall it can be concluded that a point mass model is a valid approximation to model the flight dynamics.
