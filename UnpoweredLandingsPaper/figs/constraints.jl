...
function constraints()
    ...
    init = nothing
    sol_min = solve_ocp(
        dc_min_speed_ground(u₀, pa; Tmin=3.0, Tmax=3.001); init, solver=:madnlp, display=false
    )
    ...
    sols1 = map(γs1) do γ
        solve_ocp(
            dc_min_speed_constrain_td_angle(u₀, pa, γ; Tmin=Tsol_min, Tmax=Tsol_min+0.001); init=sol_min.sol, solver=:madnlp, display=false
        )
    end
